...
def find_formant(file_path):
    ...
    for file_num, soundpath in enumerate(file_list, 1):
        try:
            ...
            to_intensity = Sound.to_intensity(time_step=0.1) # time_step초 당 intensity 변환
            ...
            df['intensity'] = df['times'].map(lambda x: to_intensity.get_value(time=x))

            df['pitch_slope'] = voice_pitch.get_mean_absolute_slope()
...
if find_formant('C:/Users/KimTaeGyuPC/Desktop/test_data2/') != 0:
